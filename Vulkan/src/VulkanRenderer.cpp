...
void createImage(VkPhysicalDevice, VkDevice, uint32_t, uint32_t, VkFormat, VkImageTiling, VkImageUsageFlags, VkMemoryPropertyFlags, VkImage&, VkDeviceMemory&);
...
VkFormat findSupportedFormat(const std::vector<VkFormat>&, VkPhysicalDevice, VkImageTiling, VkFormatFeatureFlags);
...
void VulkanRenderer::createInstance()
{
	...
	if (!areAllExtensionsSupported(requiredExtensions, extensions))
	{
...
void VulkanRenderer::loadModel()
{
	...
	if (!tinyobj::LoadObj(&attribute, &shapes, &materials, &error, MODEL_PATH.c_str())) 
	{
...
static bool areAllExtensionsSupported(const std::vector<const char*>& requiredExtensions, const std::vector<VkExtensionProperties>& extensions)
{
...
static bool checkValidationLayerSupport()
{
	...
	for (const char* layerName : validationLayers)
	{
		...
		if (!layerFound)
		{
			std::cerr << "Missing required layer: " << layerName << std::endl;
			return false;
		}
	}
...
VkFormat findSupportedFormat(const std::vector<VkFormat>& candidates, VkPhysicalDevice device, VkImageTiling tiling, VkFormatFeatureFlags features)
{
